...
const TempoSelector: React.FC = () => {
    ...
    return (
        <div className="mb-8">
            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 uppercase tracking-widest">{t.selectTempo}</label>
            <div className="grid grid-cols-3 gap-3">
                {tempos.map((mode) => (
                    <button
                        key={mode.id}
                        onClick={() => updateSettings({ tempo: mode.id })}
                        className={`py-3 rounded-2xl font-bold text-sm transition-all duration-300 transform active:scale-95 ${settings.tempo === mode.id
