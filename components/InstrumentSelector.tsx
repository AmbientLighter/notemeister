...
const InstrumentSelector: React.FC = () => {
    ...
    return (
        <div className="mb-8">
            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 uppercase tracking-widest">{t.selectInstrument}</label>
            <div className="grid grid-cols-2 gap-3">
                {instruments.map((inst) => (
                    <button
                        key={inst.id}
                        onClick={() => updateSettings({ instrument: inst.id })}
                        className={`py-4 px-3 rounded-2xl font-bold transition-all duration-300 flex items-center justify-center gap-3 transform active:scale-95 ${settings.instrument === inst.id
